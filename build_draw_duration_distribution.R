build_draw_duration_distribution <- function(df,new.data.window = NULL) {
  # simulate data until status column available
  df$status <= "success"
  ...
  cube.id <- grep("eCube",names(df))  
  cube.name <- substr(df[1,cube.id],start=0,40)
  key.id <- grep("key",names(df))  
  key.name <- df[1,key.id]
  user <- c(unique(df$AdminEmail))[[1]]
  # if (length(user) == 2)
  # {
  #   print (user)  
  # }
  print(paste("Processing Cube : ", cube.name,sep=""))
  
  # handle division of data to historical and new
  if (!is.null(new.data.window))
  {
    # write logic for a predefined new data time window
    #df$timestamp <- as.POSIXlt(df$Date) 
    cutoff.date <- unique(max(df$Date)) - new.data.window
    baseline.df <- subset(df, Date <= cutoff.date)
    newdates.df <- subset(df, Date > cutoff.date)
    minDate <- min(as.Date (newdates.df$Date))
    maxDate <- max(as.Date (newdates.df$Date))
    
  } else{
    df.sorted <- df[with(df, order(df$Date)), ]
    num.row <- nrow(df.sorted)
    baseline.id <- ceiling(num.row*0.8)
    baseline.df <- as.data.frame(df.sorted[1:baseline.id,c("duration","Date")]); names(baseline.df)[1] <- "duration"
    newdates.df <- as.data.frame(df.sorted[(baseline.id+1):num.row,c("duration","Date","status")]);names(newdates.df)[1] <- "duration"
    minDate <- min(as.Date (newdates.df$Date))
    maxDate <- max(as.Date (newdates.df$Date))
  }
  ...
  file.name <- str_replace_all(cube.name, "[^[:alnum:]]", " ")
